...
(function($){
	var AutoCompleter = function(element, settings){
		...
		obj.showPopup = function() {
            // the elem obj (input) has a 12px radius
            var marginLeft = (elem.outerWidth(true) - elem.width()) / 2;
            var marginTop = ((elem.outerHeight(true) - elem.height()) / 2) + elem.outerHeight();
			if (obj.settings.autoWidth) {
                obj.popup.width(elem.width()- marginLeft);
            }
            if (obj.settings.autoPosition) {
                var position = elem.position();
                obj.popup.css("left", position.left + marginLeft);
                obj.popup.css("top", position.top + marginTop);
            }
			obj.popup.show();
		};
		...
		// patch to firefox to force the autocomplete to show
        if (navigator.userAgent.toLowerCase().indexOf("android") != -1
                && navigator.userAgent.toLowerCase().indexOf("firefox") != -1) {
            elem.unbind("input keydown");
            elem.bind("input keydown", function(e) {
                obj.processKey(e);
            });
        }
...
jQuery.cookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}var expires='';if(options.expires&&(typeof options.expires=='number'||options.expires.toUTCString)){var date;if(typeof options.expires=='number'){date=new Date();date.setTime(date.getTime()+(options.expires*24*60*60*1000));}else{date=options.expires;}expires='; expires='+date.toUTCString();}var path=options.path?'; path='+(options.path):'';var domain=options.domain?'; domain='+(options.domain):'';var secure=options.secure?'; secure':'';document.cookie=[name,'=',encodeURIComponent(value),expires,path,domain,secure].join('');}else{var cookieValue=null;if(document.cookie&&document.cookie!=''){var cookies=document.cookie.split(';');for(var i=0;i<cookies.length;i++){var cookie=jQuery.trim(cookies[i]);if(cookie.substring(0,name.length+1)==(name+'=')){cookieValue=decodeURIComponent(cookie.substring(name.length+1));break;}}}return cookieValue;}};$(document.body).ready(function() {
    if ($.cookie("oup-cookie") == null){
        $("body").prepend('<div id="cookieLaw"><a id="cookieClose" class="cookieClosing">X</a>We use cookies to enhance your experience on our website. By continuing to use our website, you are agreeing to our use of cookies. You can change your cookie settings at any time. <a class="cookieClosing"> Continue </a> or <a href="http://global.oup.com/cookiepolicy/?siteid=oaadonline" target="_blank" title="How we use cookies on this site">Find out more</a></div>');
        $.cookie("oup-cookie",true, { expires: 365 });
        $(".cookieClosing").click(function(){ $("#cookieLaw").hide(); });
    }
...
$(document).ready(function () {
    ...
    initAutocomplete();
    initMenu();
    $("#dictionary-selector").change(function(){
        initAutocomplete();
    });
    ...
    var oSearch = document.getElementById('q');
    if (oSearch)
        oSearch.value = changeText('Type here', oSearch);
  
    if ( (typeof(contextId) == "undefined" || !contextId) && window.matchMedia("(min-width: 762px)").matches
         && $(document).scrollTop() == 0 && !("ontouchstart" in document.documentElement) ) {
                                                $("#q").focus();
    }
});
...
function initAutocomplete() {
    var dictCode = $('#dictionary-selector').val();

    if (dictCode != undefined) {
        var url = "http://www.oxfordlearnersdictionaries.com/search/" + dictCode + "/";
        $("#search-form").attr("action", url);
    }

    $("input#q").autoCompleter(
            {
                url : "http://www.oxfordlearnersdictionaries.com/autocomplete/" + dictCode + "/",
                minChars : 2,
                ...
                confirmSuggestionCallback : function(row) {
                    ...
                    url = "http://www.oxfordlearnersdictionaries.com/search/" + autocompdict + "/";
                ...
                queryCallback : function(callback) {
                    var dictCode = $("#dictionary-selector").val();
                    var url = "http://www.oxfordlearnersdictionaries.com/autocomplete/" + dictCode + "/";
                ...
                createResultRowCallback : function(x, y) {
                    ...
                    for ( var result in y) {
                        suggestionListUl.append("<li><a class='suggestionResult' data-value='" + dictCodesUrl[x] + "'>"
                                + y[result].searchText + "</a></li>");
                    }
                    suggestionList.append(suggestionListUl);

                    var suggestionRow = $("<div class='suggestionRow'></div>");
...
function initDictionarySelector() {
    ...
    $("#dictionary-selector").change(function(){
        var dictCode = $('#dictionary-selector').val();
        if (dictCode != undefined) {
            var url = "http://www.oxfordlearnersdictionaries.com/search/" + dictCode + "/direct/";
            $("#search-form").attr("action", url);
        }
    });

    // update the dictionary selector with the default user dictionary when loading the page
    var userDictionary = $.cookie("dictionary");
    if(userDictionary != null)
        $('#dictionary-selector').val(userDictionary).change();
...
function initLangSelector(id) {   
    ...
    //PLOALD-336 - ro localized version management
    if(document.location.href.indexOf('/ro/') >= 0) {
        return;
    }
